...
This document describes the Termux-specific patches applied to the official OpenAI Codex CLI so that it works well on Android Termux (arm64 + armv7).
...
```rust
// Line 56: Update URL
const LATEST_RELEASE_URL: &str = "https://api.github.com/repos/sirvkrm/codex-vkrm/releases/latest";
...
**File**: `codex-rs/tui/src/update_action.rs` + `codex-rs/Cargo.toml`
**Lines Modified**: 4
**Date Applied**: 2025-11-05 (introduced in 0.55.x, revalidated in 0.60.1)
...
**Two issues:**
1. **Wrong package**: Uses `@openai/codex` instead of `@sirvkrm/codex-cli-termux`
...
```rust
// update_action.rs
UpdateAction::NpmGlobalLatest => ("npm", &["install", "-g", "@sirvkrm/codex-cli-termux@latest"]),
UpdateAction::BunGlobalLatest => ("bun", &["install", "-g", "@sirvkrm/codex-cli-termux@latest"]),
```
...
```bash
npm install -g @sirvkrm/codex-cli-termux@latest
```

**Error seen:**
```
Updating Codex via `npm install -g @sirvkrm/codex-cli-termux@latest`...
env: 'node': Permission denied
Error: failed with status exit status: 126
```
...
```bash
# Install old version:
npm install -g @sirvkrm/codex-cli-termux@0.58.4-termux
...
- `@sirvkrm/codex-cli-termux@0.61.0-termux` - deprecated
- `@sirvkrm/codex-cli-termux@0.60.1-termux` - deprecated
...
- ✅ Patch #4: Auto-update URL (sirvkrm/codex-vkrm)
- ✅ Patch #5: Version parser (-termux suffix)
- ✅ Patch #6: NPM package name (@sirvkrm/codex-cli-termux)
...
**Platform**: Android Termux (arm64 + armv7)
