...
2. Check `bin/codex.js` wrapper exists
3. Check `bin/codex-exec.js` wrapper exists
4. Check `bin/android-arm64/` payload exists
5. Check `bin/android-armv7/` payload exists

**Expected**: wrappers plus both ABI payload folders present

**Verify**:
```bash
ls -la $(npm root -g)/@sirvkrm/codex-cli-termux/bin/
# Should show: codex.js, codex-exec.js, android-arm64/, android-armv7/
```
...
**Note**: Run after `npm install -g @sirvkrm/codex-cli-termux`
